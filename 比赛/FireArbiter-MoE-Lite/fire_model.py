...
DEFAULT_OWLVIT_POSITIVE_PROMPTS = (
    "visible flame", "open flame", "active fire", "real fire", "burning flames", "fire",
    "visible combustion flame", "burning object with visible flames", "burning material with flames",
    "small visible flame", "small flame", "tiny flame", "faint flame", "large visible flame",
    "spreading flames", "orange flame", "yellow flame", "blue flame", "white hot flame",
    "match flame", "lighter flame", "candle flame", "gas stove flame", "gas burner flame",
    "torch flame", "campfire flames", "fireplace flames", "flaming debris",
    "burning wood with flames", "burning grass", "burning vehicle", "building on fire",
    "forest fire", "industrial fire",
    # 火焰视频/视觉素材：使用图像内容式描述，避免抽象的“素材”单词。
    "video footage of fire", "video footage of flames", "fire stock footage",
    "animated flame effect", "flame visual effect", "flame overlay",
)

DEFAULT_OWLVIT_NEGATIVE_PROMPTS = (
    "electric lamp", "ceiling lamp", "street light", "led light", "neon light",
    "decorative light", "stage light", "orange lamp", "red lamp", "car headlight",
    "car tail light", "vehicle brake light", "traffic light", "traffic cone",
    "orange traffic cone", "road warning light", "reflective road marker",
    "orange road barrel", "yellow warning sign", "light reflection", "glass reflection",
    "metal reflection", "headlight glare", "lens flare", "overexposed bright area",
    "sunlight glare", "bright illuminated window", "sunset sky", "sunrise sky",
    "orange sky", "red sky", "sun behind clouds", "sunlight through clouds",
    "orange object", "red object", "yellow object", "orange fabric", "red fabric",
    "yellow flower", "orange leaves", "autumn leaves", "orange safety vest",
    "video subtitle", "white subtitle text", "image watermark", "bright text overlay",
    "screen graphic overlay", "illuminated screen", "television screen",
    "computer monitor", "bright signboard", "colorful advertisement",
    "led display", "led display screen", "led message board", "electronic scrolling sign",
    "red led text", "scrolling red led text", "red text on an led display",
    "traffic cone",
    "orange traffic cone",
    "road cone",
    "safety cone",
...
DEFAULT_OWLVIT_SUSPICIOUS_PROMPTS = (
    "lighter", "cigarette lighter", "pocket lighter", "gas lighter", "disposable lighter",
    "butane lighter", "match", "matchstick", "wooden match", "lit match",
    "box of matches", "match head", "glowing match head", "candle", "candle wick",
    "torch", "blowtorch", "gas torch", "ignition device", "fire starter",
    "fire starter stick", "gas stove burner", "portable gas stove", "camping stove",
    "fireplace", "fire pit", "barbecue grill", "charcoal grill", "incense stick",
    "burning wooden stick", "smoldering wooden stick", "burning cigarette",
    "cigarette ember", "firework", "signal flare", "burning fuse", "smoke",
    "thin smoke", "heavy smoke", "white smoke", "gray smoke", "black smoke",
    "rising smoke", "smoke plume", "smoldering material", "smoking object",
    "smoking electrical device", "battery smoke", "ember", "glowing ember", "hot coal",
    "glowing charcoal", "ash", "burnt object", "charred object", "scorched surface",
    "hot metal", "red hot metal", "glowing metal", "overheated object", "electrical spark",
    "flying sparks", "spark shower", "welding sparks", "welding arc", "electric arc",
    "damaged electrical wire", "overheated wire", "sparking electrical wire",
    "burnt electrical outlet", "fire-like bright region", "orange glowing region",
    "yellow glowing region", "flickering bright spot",
)
...
class LiteArbiter(nn.Module):
    def __init__(self, num_experts: int):
        super().__init__()
        input_dim = 2 + 2 + 1 + 3 + 3 + num_experts * 2
        self.weight_head = MLP(input_dim, 96, 2, dropout=0.05)
        self.semantic_scale = nn.Parameter(torch.tensor(-1.5))
        self.logit_scale = nn.Parameter(torch.tensor(1.0))
        self.logit_bias = nn.Parameter(torch.tensor(0.0))

    @staticmethod
    def _map_stats(attention: torch.Tensor) -> torch.Tensor:
        flat = attention.flatten(1).clamp(1e-6, 1.0 - 1e-6)
        return torch.cat([
            flat.mean(dim=1, keepdim=True),
            flat.amax(dim=1, keepdim=True),
            (-(flat * flat.log())).mean(dim=1, keepdim=True),
        ], dim=1)

    def forward(self, branches: BranchPair, context: FusionContext):
        global_branch, local_branch = branches.global_branch, branches.local_branch
        descriptor = torch.cat([
            global_branch.logit, local_branch.logit,
            global_branch.confidence, local_branch.confidence,
            context.semantic.confidence, context.semantic.scores,
            self._map_stats(context.semantic.attention),
            context.expert.prior_weights, context.expert.posterior_weights,
        ], dim=1)
        weights = torch.softmax(self.weight_head(descriptor), dim=1)
        raw_logit = (
                weights[:, 0:1] * global_branch.logit
                + weights[:, 1:2] * local_branch.logit
                + torch.sigmoid(self.semantic_scale) * context.semantic.confidence * context.semantic.logit
        )
        logit = self.logit_scale.clamp(0.25, 4.0) * raw_logit + self.logit_bias
        mix_map = (
                weights[:, 0:1, None, None] * global_branch.attention
                + weights[:, 1:2, None, None] * local_branch.attention
                + torch.sigmoid(self.semantic_scale) * context.semantic.confidence[:, :, None, None]
                * context.semantic.attention
        )
        mix_map = mix_map / mix_map.amax(dim=(2, 3), keepdim=True).clamp_min(1e-6)
